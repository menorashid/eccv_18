...
class Vgg_Capsule(Dynamic_Capsule_Model_Super):
    ...
    def __init__(self,n_classes,pool_type='max',r=3,class_weights=None, reconstruct = False, loss_weights = None, std_div = None):
        ...
        vgg_base = list(torch.load('models/pytorch_vgg_imagenet_just_conv.pth'))
        ...
        if self.reconstruct:
            ...
            if hasattr(self, 'std_div') and self.std_div is not None:
                for dim_curr in range(3):
                    # print self.std_div.data.type()
                    # print images_copy.data.type()
    ...
    def log_margin_loss(self,  classes,labels):
        if self.reconstruct:
            images = classes[2]
            reconstructions = classes[1]
            classes = classes[0]
...
class Network:
    ...
    def get_lr_list(self, lr):
        ...
        lr_list =[]
        module_list = [self.model.vgg_base,self.model.vgg_base_second,self.model.features]
...
def main():
    ...
    net = Network(n_classes = 12, pool_type = 'nopool', init = False, reconstruct = True)
    print net.model

    n_classes = 7
    features = list(net.model.features)
    features[1] = CapsuleLayer(n_classes, 32, 8, 32, kernel_size=6, stride=1, num_iterations=3)
    net.model.features = nn.Sequential(*features)
        
    if net.model.reconstruct:

        if net.model.reconstruct:
            net.model.decoder = nn.Sequential(
                nn.Linear(32 * n_classes, 512),
                nn.ReLU(inplace=True),
                nn.Linear(512, 1024),
                nn.ReLU(inplace=True),
                nn.Linear(1024, 56*56*3),
            )
            net.model.upsampler = nn.Upsample(size=(224,224), mode='bilinear')
