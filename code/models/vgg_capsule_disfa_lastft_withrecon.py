...
class Vgg_Capsule_Disfa_Recon(Dynamic_Capsule_Model_Super):
    ...
    def __init__(self,n_classes,loss,in_size = 224, r=3,):
        ...
        self.vgg_base = list(torch.load('models/pytorch_vgg_face_just_conv.pth'))
        self.vgg_last = nn.Sequential(*self.vgg_base[-2:])
        self.vgg_base = nn.Sequential(*list(self.vgg_base)[:-2])
...
class Network:
    ...
    def get_lr_list(self, lr):
        lr_list =[]
        for lr_curr,param_set in zip(lr,[self.model.vgg_base,self.model.vgg_last,self.model.features,self.model.decoder]):
            if lr_curr==0:
