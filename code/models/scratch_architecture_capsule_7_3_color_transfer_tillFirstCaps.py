...
import math
from models import scratch_architecture_capsule_7_3_color as sac
...
class Vgg_Capsule(Dynamic_Capsule_Model_Super):
    ...
    def __init__(self,n_classes,pool_type='max',r=3,class_weights=None, reconstruct = False, loss_weights = None, std_div = None):
        ...
        if std_div is not None:
            self.std_div = torch.Tensor(std_div).cuda()
        ...
        # vgg_base = list(torch.load('models/pytorch_vgg_face_just_conv.pth'))
        # self.vgg_base = nn.Sequential(*vgg_base[:24])
        # self.vgg_base_second = nn.Sequential(*vgg_base[24:]+[nn.Dropout2d(p=0.5)])
        model_transfer_path = 'models/model_best_bp4d_scratch_color_dict.pt'
        model_transfer = sac.Vgg_Capsule(n_classes = 12,reconstruct = True)
        model_transfer.load_state_dict(torch.load(model_transfer_path))
        # model_transfer = torch.load(model_transfer_path)
        self.vgg_base = model_transfer.vgg_base
        ...
        self.features = []
        self.features.append(list(model_transfer.features)[0])
            # CapsuleLayer(32, 1, 128, 8, kernel_size=7, stride=3, num_iterations=r))
    ...
    def forward(self, data, y = None,return_caps = False):
        ...
        if self.reconstruct:
            ...
            reconstructions = reconstructions.view(reconstructions.size(0),3,32,32)
...
def main():
    ...
    net = Network(n_classes = 8, pool_type = 'max', init = False, reconstruct = True)
    print net.model
    ...
    print output.size(),recon.size(),data.size()
