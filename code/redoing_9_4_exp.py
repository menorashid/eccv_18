...
def hack():
    ...
    dir_curr = '../experiments/khorrami_capsule_7_3_bigclass3/ck_96_train_test_files_0_reconstruct_True_True_all_aug_margin_False_wdecay_0_600_exp_0.96_350_1e-06_0.001_0.001_0.001'
    out_file = os.path.join(dir_curr,'log.txt')
    val_lines = []

    for model_to_test in range(0,600,30)+[599]:
        val_line = util.readLinesFromFile(os.path.join(dir_curr,'results_model_'+str(model_to_test),'log.txt'))[-1]
        val_lines.append(val_line)

    util.writeFile(out_file,val_lines)
